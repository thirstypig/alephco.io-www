...
The single highest-leverage move is to stop working per product and start working per
**attribute group**. Most obligations key on something shared:

- a **category** (children's product, food, cosmetic) decides which regime applies
- a **material or component** decides which substance rules bite
...
The recurring obligations run on a handful of clocks:

- **Annual-ish**: children's product periodic testing on your chosen interval
  ([§1107.21](https://www.ecfr.gov/current/title-16/part-1107)); Minnesota PFAS updates each
  1 February ([§116.943](https://www.revisor.mn.gov/statutes/cite/116.943))
- **Three-yearly**: FSVP supplier reevaluation
...
2. **Which certificates name no specific rule?** §1110.11(a)(2) requires each listed
   separately, and this defect is usually catalogue-wide because it came from one template.
...
**Do not test everything.** Testing is a screening and rebuttal tool, not a compliance
demonstration — see
[fluorine testing](/blog/total-organic-fluorine-testing-when-you-need.html). Get the
declaration first; it is cheaper and it is what filings actually require.

**Do not warn on everything.** Over-warning has real commercial cost, and for Prop 65 the
[exposure route](/blog/exposure-assessment-without-a-toxicologist.html) may put you below
the level.
...
1. Classify the catalogue by category, material, supplier and destination. Once.
2. Fix the template defects — they are catalogue-wide, so one fix clears hundreds.
3. Get dated declarations per material group, not per SKU.
4. Put the recurring clocks in a calendar with an owner.
...
and talk to counsel about your catalogue.*
